...
2. **More Effective & Efficient Capital Raising**
...
2. To avoid the typical General Partner/Limited Partner classification and Investment Advisor liabilities by relying on collective decision making, which also helps distribute risk, reward, and costs.

Additionally, The Fish Network offers investors an interactive & social voting experience, using reward reputation points(FISH points), which helps investors build their network, portfolio, and track record on-chain.

_**Coordination:**_ A Fish School Organizer will be appointed in each newly formed LLC(i.e. Fish School).

* To provide accountability over the formation of the LLC and sourcing of accounting, legal, and admin services.
...
1. Investor (Fish) commits capital → KYC/KYB → custody/escrow initialized.
2. Organizer curates deals → community diligence → on‑chain vote.
...
* **VC Platform(Fish Shoal Leader):** Multi‑manager program, LP engagement, &lt;$25k investor activation, follow‑on syndication.
...
Fish Network flips the syndicate & crowdfunding model on its head. Instead of chasing investors for each deal, when most of them drop out anyway, we introduce a new capital deployment model, where the capital has already been raised and locked in smart contract escrow. Rather than using a traditional SPV, we introduce the concept of a Fish School, where any emerging manager can launch an investment club to convert their passive audience into active co-investors.
